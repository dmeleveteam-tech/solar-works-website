/**
 * Seed the CMS collections (projects / testimonials / faqs / solutions) with the content the
...
const solutions: Array<{
  slug: "grid-tied" | "hybrid-with-battery" | "commercial-farm-carport" | "solar-carport"
  name: string
  forWho: string
  summary: string
  highlights: string[]
...
}> = [
  {
    ...
    highlights: ["Lowest upfront cost per kW", "Fastest payback period", "Net-metering ready"],
    image: "",
  },
  {
    ...
    highlights: ["Backup power during brownouts", "Use solar energy after sunset", "Smart load prioritization"],
    image: "",
  },
  {
    ...
    highlights: ["Designed around your load profile", "Roof, ground-mount, or carport", "Measurable operating-cost reduction"],
    image: "",
  },
  {
    slug: "solar-carport",
    ...
    highlights: ["Dual-purpose: power + shade", "EV-charging ready", "No roof space required"],
    image: "",
  },
]
...
async function main() {
  ...
  // Solutions — keyed by slug (fixed set of 4).
  let s = 0
  for (const [i, sol] of solutions.entries()) {
    await db.collection("solutions").updateOne(
      { slug: sol.slug },
      {
        $set: { ...sol, published: true, sortOrder: i, updatedAt: now },
        $setOnInsert: { createdAt: now },
      },
      { upsert: true },
    )
  ...
  console.log(`✓ Solutions seeded: ${s}`)
