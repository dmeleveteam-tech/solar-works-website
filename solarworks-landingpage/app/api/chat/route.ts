 ...
 * explicit consent — let the model call the `save_lead` tool, which we validate
 * and forward to the platform inbox as a `chatbot` lead.
 *
 * The Groq key lives only here, never in the browser. The endpoint degrades
 * gracefully: with no key it returns a friendly hand-off message rather than an
 * error, mirroring how the rest of the site behaves before its keys are set.
 */
...
export async function POST(req: Request) {
  ...
  // No key, or no inbox to save into → hand off to humans gracefully.
  if (!GROQ_API_KEY || !leadIntakeConfigured()) {
